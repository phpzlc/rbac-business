...
class RoleBusiness extends AbstractBusiness
{
    public function validator($class): bool
    {
        if(parent::validator($class)){
           if($class instanceof Role){
               $roleId = $this->em->getRepository(Role::class)->findAssoc([
                  'tag' => $class->getTag(),
               ...
               $role = $this->em->getRepository(Role::class)->findAssoc([
                   'name' => $class->getName(),
                   'platform' => $class->getPlatform()
               ]);
    ...
    public function del(Role $role, $is_flush = true)
    {
        ...
        $useRoles = $this->em->getRepository(Role::class)->findAll(['contain_role_ids' . Rule::RA_LIKE => $role->getId()]);
        foreach ($useRoles as $useRole){
            $this->removeContainRole($useRole, $role, false);
        }

        //获取角色被哪些用户使用过
        $userRoles = $this->em->getRepository(UserAuthRole::class)->findAll(['role_id' => $role->getId()]);
